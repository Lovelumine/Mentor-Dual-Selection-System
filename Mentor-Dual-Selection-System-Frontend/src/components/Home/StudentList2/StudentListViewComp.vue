...
function isStudentGradeSec(): string | null{
  const currentYearMonth = getCurrentYearMonth();
  const [currentYear, currentMonth] = currentYearMonth.split('-').map(Number);
  let entryYear = new Date().getFullYear() - 1;
  if (entryYear === (currentYear - 1) && 9 <= currentMonth) { // 获取年和当前年相等，但月份九月以后，就是获取年的学年
    console.log(1, entryYear);
    return entryYear.toString().padStart(4, '0');
  } else if ((entryYear + 1) === currentYear && currentMonth <= 8) { // 获取年和当前年相等，但月份八月以前，就是获取年-1的学年
